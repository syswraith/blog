...
1. On analyzing the script, we can see that by inputting an address in hexadecimal format, we can jump to anywhere in the script.
1. On further analysis, we come across the following mechanism implemented by the OS (in my case Linux)- [Address Space Layout Randomization (ASLR)](https://ctf101.org/binary-exploitation/address-space-layout-randomization/)
...
4. This also implies the existence of two types of addresses, both of which refer to the same function at different times:
   ...
   1. Runtime address
   1. Static address
4. By running `nm` we can get the static addresses of `main()` and `win()`

![running nm pie time.png](/CTFs/images/running-nm-pie-time.png)
...
![connecting pie time.png](/CTFs/images/connecting-pie-time.png)

9. We'll input this address as the dynamic main address in our script

![pie time script.png](/CTFs/images/pie-time-script.png)

10. And we have the flag :)

![pie time flag.png](/CTFs/images/pie-time-flag.png)
