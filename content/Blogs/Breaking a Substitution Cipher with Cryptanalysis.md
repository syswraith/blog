...
1. Plaintext is passed to an **encryption function**, which is a set of rules for transforming the message.
1. The function also takes a **key**, known only to the sender and receiver.
1. Using the key, the function produces the **ciphertext** (encrypted message).
...
1. **Look for repeating patterns** — double letters, common suffixes (`-ing`, `-ed`).
1. **Track mappings in a table.** Sometimes one may be able to bruteforce the key used to encrypt the message, and then break the cipher that way.
1. **Analyze the frequency of letters.** For example, letters like `E`, `T`, `A` appear often in English.
1. A good resource for more information on cryptography is **The Code Book by Simon Singh**.
